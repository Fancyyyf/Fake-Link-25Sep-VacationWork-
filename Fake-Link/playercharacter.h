...
public:
    ...
    // 返回是否实际移动成功
    void Move(double dx, double dy, const QVector<QVector<int>> &board);
...
private:
    ...
    bool canOccupyRect(double left, double top, double right, double bottom,
    ...
    // 玩家在逻辑坐标系中的占用尺寸（以格子为单位，默认小于1）
    double pixwid  = 0.8;
    double pixhei = 0.8;

    double speed = 0.15; // 单位：逻辑单位/帧（可由 MainWindow 调整）

    // 方向图片
    QPixmap imgUp, imgDown, imgLeft, imgRight;
